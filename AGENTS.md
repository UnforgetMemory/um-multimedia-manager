...
**UMM（Unified Multimedia Manager）** — Chrome 扩展（Manifest V3），统一管理豆瓣/IMDb/NeoDB/TMDB/Bangumi/PT 站的观影收听记录，支持跨平台同步、PT 种子淡化、WebDAV 备份。Vue 3 + TypeScript + WXT + Tailwind CSS v4 + reka-ui。当前版本 5.17.1。
...
| `sehuatang-early.content/` + `sehuatang-main.content/` | 新 Sehuatang 注入（ADR-024）：early 建壳 + 首帧背景（列表页/搜索页/首页均建壳），main 接管 overlay 编排——**风控页（DOM 双标记检测，优先级最高）**/ 列表页（`runSehuatangOverlayApp`）/ 搜索页（`runSehuatangSearchApp`，仅 dimmer 不落库）/ 首页（`runSehuatangIndexApp`，导航层不触已看库）；帖子详情页走静默记录，不建 overlay、不注入 UI。**不经 legacy 管线** |
