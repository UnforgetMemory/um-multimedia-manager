...
console.log('[UMM Content] Chrome runtime available:', !!chrome?.runtime?.id)
...
let currentIdentity = Identity.fromUrl(window.location.href)
...
async function init() {
  console.log('[UMM Content] Initializing...')
  
  if (!chrome?.runtime?.id) {
    console.error('[UMM Content] Chrome runtime not available!')
    return
  }
  ...
  try {
    ...
    console.error('[UMM Content] ❌ Initialization failed:', error)
  }
}
...
async function loadCurrentRecord() {
  ...
  try {
    ...
    console.log('[UMM] Current record:', currentRecord)
  } catch (error) {
    console.error('[UMM] Failed to load record:', error)
  }
}
...
async function handleDoubanDetailPage() {
  ...
  if (isPageDone) {
    ...
    console.log('[UMM Debug] Saving record:', {
...
function createFloatingPanel() {
  ...
  console.log('[UMM] Floating panel created')
}
...
async function saveRecord() {
  ...
  try {
    ...
    console.error('[UMM] Save failed:', error)
    showNotification('❌ 保存失败')
  }
}
...
function observeUrlChanges() {
  ...
  urlObserver = new MutationObserver(() => {
    if (window.location.href !== lastUrl) {
      ...
      console.log('[UMM] URL changed:', lastUrl)
...
function observeThemeChanges() {
  ...
  themeChangeListener = (e: MediaQueryListEvent) => {
    console.log('[UMM] Theme changed:', e.matches ? 'dark' : 'light')
